...
png("./Exploratory-Data-Analytics/plot6.png", width=800,height=400)
  g <- ggplot(y, aes(x = Year, y = value, fill = City, color=ifelse(City == "Baltimore", 'red', 'black')))+scale_color_identity()
  g <- g + geom_bar(stat = "identity", position = position_dodge()) + facet_grid( ~ Sector) + geom_smooth(method = "lm", se = FALSE)
  g + labs(title = "PM2.5 Emissions from Motor Vehicles \nin Baltimore and Los Angeles by sector by year.") + labs(x = "Year", y = "PM2.5 Emission")
